...
os.environ["OPENAI_API_KEY"] = "a9d77ec5e2da424c9e3724acb3dc14be.6jFsWJ6dNgvIzL4I"
os.environ["ZHIPU_API_KEY"] = "a9d77ec5e2da424c9e3724acb3dc14be.6jFsWJ6dNgvIzL4I"
class OpenAIChat():
    """openAI chatGPT 对话
    """
    ...
    def get_response_stream(self, text: str):
        ...
        self.update_chat_messages('user', text)
        # a ChatCompletion request
        print(f"get_message steam: {self.get_messages()}")
        response = self.client.chat.completions.create(
            model=self.model,
            messages=self.get_messages(),
            temperature=self.temperature,
    ...
    def sdecode_chunk(self, chunk, server):
            """解码对话流式响应数据,分句返回回答结果
            Args:
                chunk   流式响应response的元素
            Note:
